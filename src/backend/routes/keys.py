...
def sign_route(
    request: SignatureRequest, principal: Principal = Depends(get_current_principal)
) -> SignatureResponse:
    _ = principal
...
@router.post("/verify")
def verify_route(
    request: SignatureVerifyRequest, principal: Principal = Depends(get_current_principal)
) -> dict:
    _ = principal
